...
// GET - List all stories for a girl (with optional pagination)
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const countResult = await db.execute({
      sql: 'SELECT COUNT(*) as total FROM stories WHERE girl_id = ?',
      args: [girlId]
    });
    ...
    const result = await db.execute({
      sql: 'SELECT * FROM stories WHERE girl_id = ? ORDER BY order_index ASC, created_at DESC LIMIT ? OFFSET ?',
