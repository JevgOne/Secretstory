...
interface DivkyClientProps {
  initialGirls: Girl[];
  locale: string;
}

export default function DivkyClient({ initialGirls, locale }: DivkyClientProps) {
  ...
  // Use server-fetched data - no client-side fetch needed!
  const girls = initialGirls;
  const loading = false;

  console.log('[DIVKY CLIENT] Rendered with girls:', girls.length);
                    ...
                    <article className="card">
                      ...
                      <div className="card-info">
                        <div className="card-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', flexWrap: 'nowrap' }}>
                          ...
                          {timeRange && <span className={`time-badge ${isWorking ? 'available' : 'tomorrow'}`} style={{ fontSize: '0.55rem', padding: '2px 5px', whiteSpace: 'nowrap' }}>{timeRange}</span>}
                        </div>
                        ...
                        {isWorkingToday && (
                          <div className="card-location-wrapper" style={{ marginTop: '5px', textAlign: 'center' }}>
