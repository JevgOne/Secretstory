...
export default function ProfileDetailPage({
  ...
  const fetchProfile = async () => {
    try {
      // Use server-side pre-fetched data if available (FASTER!)
      if (initialData?.girl) {
        setProfile(initialData.girl);
        trackProfileView(initialData.girl.id);

        // Only fetch additional data (online girls, reviews)
        const [onlineResponse, reviewsResponse] = await Promise.all([
          fetch(`/api/girls/online-today?limit=4`).catch(() => null),
          fetch(`/api/reviews?status=approved&girl_id=${initialData.girl.id}`).catch(() => null)
        ]);

        if (onlineResponse) {
          const onlineData = await onlineResponse.json();
          if (onlineData.success) {
            setOnlineGirls((onlineData.girls || []).filter((g: Girl) => g.id !== initialData.girl.id));
          }
        }
          ...
          if (reviewsData.success && reviewsData.reviews) {
            ...
            reviews.forEach((review: any) => {
              ...
              reviewTags.forEach((tag: string) => {
                if (TAG_EMOJIS[tag]) {
                  if (!tags[tag]) {
                    tags[tag] = { count: 0, emoji: TAG_EMOJIS[tag] };
                  }
                  tags[tag].count++;
                }
              });
            ...
            const topTags = Object.entries(tags)
              ...
              .slice(0, 3)
              .map(([name, data]) => ({ name, count: data.count, emoji: data.emoji }));
            // topTags calculated (reviewTags state handling removed for simplicity)
          }
        }
        return;
      }

      // Fallback: Client-side fetch (slower, for backward compatibility)
