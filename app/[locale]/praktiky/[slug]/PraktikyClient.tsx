...
interface PraktikyClientProps {
  girls: Girl[];
  serviceSlug: string;
  serviceName: string;
}

export default function PraktikyClient({ girls, serviceSlug, serviceName }: PraktikyClientProps) {
  ...
  return (
    <div className="container" style={{ padding: "100px 20px", maxWidth: "1200px", margin: "0 auto" }}>
      <Link href={`/${locale}/divky`} style={{ display: "inline-block", marginBottom: "2rem", color: "var(--wine)" }}>
        ...
        <div className="hashtags">
          {SERVICES.map((service) => (
            <Link
              ...
              key={service.id}
              className={`hashtag ${service.id === serviceSlug ? 'active' : ''}`}
            >
