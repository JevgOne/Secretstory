...
export default function BottomCTA({ translations }: BottomCTAProps) {
  ...
  // Fetch WhatsApp blocked status from API
  useEffect(() => {
  ...
  return (
    <>
      {/* Warning Banner - only show when WhatsApp is blocked */}
      {whatsappBlocked && translations.whatsapp_warning && (
      ...
      <div className={`bottom-cta ${isVisible ? 'visible' : 'hidden'}`}>
        ...
        <button onClick={handleWhatsApp} className={`bottom-cta-btn whatsapp ${whatsappBlocked ? 'disabled' : ''}`}>
          <svg viewBox="0 0 24 24" fill="currentColor">
          ...
          {whatsappBlocked && <span className="warning-badge">!</span>}
        </button>
      </div>
